...
import { getBrand, slugToModelInBrand, storageLabel, formatPrice, GRADE_CONFIG, getBuyBackPrice, type ConditionGrade } from "@/lib/phones";
import { Check, ArrowLeft, ChevronRight } from "lucide-react";

const batteryLabel: Record<string, string> = {
  above87: "Từ 87% trở lên", below87: "Dưới 87%",
  // legacy values
  above90: "Trên 90%", "80-90": "80–90%", "70-80": "70–80%", below70: "Dưới 70%",
};

function QuoteContent({ params }: { params: Promise<{ brand: string; modelSlug: string; storage: string }> }) {
  const { brand: brandSlug, modelSlug, storage } = use(params);
  const sp = useSearchParams();
  const battery = sp.get("battery") ?? "80-90";
  const gradeSlug = (sp.get("grade") ?? "loai-2") as ConditionGrade;
  const gradeInfo = GRADE_CONFIG[gradeSlug] ?? GRADE_CONFIG["loai-2"];
  // price from URL (already calculated), fallback to lookup
  const urlPrice = Number(sp.get("price") ?? "0");
  const price = urlPrice || getBuyBackPrice(brandSlug, modelSlug, storage, gradeSlug);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-2 text-sm text-gray-500 flex-wrap">
          ...
          <ChevronRight className="w-3 h-3" />
          <a href={`/sell/${brandSlug}/${modelSlug}/${storage}/functionality?battery=${battery}`} className="hover:text-green-500">Tình trạng</a>
          <ChevronRight className="w-3 h-3" />
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        <a
          href={`/sell/${brandSlug}/${modelSlug}/${storage}/functionality?battery=${battery}`}
        ...
        <div className="bg-gradient-to-br from-green-500 via-green-600 to-green-600 rounded-3xl p-8 text-white text-center mb-6 shadow-xl relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="text-5xl sm:text-6xl font-extrabold mb-3">{formatPrice(price)}</div>
            {/* Condition grade badge */}
            <div className="inline-flex items-center gap-1.5 bg-white/20 rounded-full px-4 py-1.5 mb-2">
              <span>{gradeInfo.emoji}</span>
              <span className="font-bold text-sm">Tình trạng: {gradeInfo.badge}</span>
            </div>
...
export default function QuotePage({ params }: { params: Promise<{ brand: string; modelSlug: string; storage: string }> }) {
  return <Suspense><QuoteContent params={params} /></Suspense>;
}
